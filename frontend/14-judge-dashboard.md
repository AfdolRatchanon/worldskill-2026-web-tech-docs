...
└── components/
    └── judge/
        ...
        └── SubmissionsTable.jsx    ← สร้างในบทนี้
```
        ...
        <tbody>
          {candidates.map((c) => (
            <tr key={c.id} className="border-b border-gray-50 hover:bg-gray-50">
              ...
              <td className="py-3 px-4">
                {c.submission_id                              // ternary: ถ้ามี submission_id → Badge, ถ้าไม่มี → "No submission"
              ...
              <td className="py-3 px-4">
                <div className="flex gap-2 items-center">
                  {c.submission_id && !c.is_confirmed && (  // && = แสดงเมื่อ ทั้งสองเงื่อนไขเป็น true
                    <Button
                      variant="ghost"
                  ...
                  {!!c.is_confirmed && (                    // !! แปลง 1/0 เป็น true/false ก่อนใช้กับ &&
...
สร้าง `src/components/judge/SubmissionsTable.jsx` — แสดง URL ที่ candidate ส่งมา (GET /api/submissions):

```jsx
// components/judge/SubmissionsTable.jsx — บทที่ 14
import Badge from '../common/Badge';
...
import SubmissionsTable from '../../components/judge/SubmissionsTable';
...
export default function JudgeDashboard() {
  ...
  useEffect(() => {
    async function fetchAll() {
      try {
        const [cfgRes, candRes, subRes] = await Promise.all([
          api.get('/config'),       // session ปัจจุบัน
          api.get('/candidates'),   // รายชื่อ + คะแนน + action buttons
          api.get('/submissions'),  // URL ที่ candidate ส่งมา
        ]);
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-5xl mx-auto p-6 space-y-6">
        ...
        <Card>
          ...
          <CandidateTable candidates={candidates} onUpdate={() => setTick(t => t + 1)} />
        </Card>
...
5. กลับมา Judge → ต้องเห็น candidate มี submission — Badge "pending" ใน Candidates card
6. ต้องเห็น **Submissions card** แสดง Frontend URL + Backend URL + Badge status ของ candidate
7. DevTools → Network → เห็น `GET /api/submissions` ทุก 5 วินาที
8. กด **Re-check** → Badge เปลี่ยนเป็น "checking" → รอ ~2.5 วินาที → เปลี่ยนเป็น "checked" + มีคะแนน
9. กด **Confirm** → เห็น "✓ Confirmed" แทนปุ่ม
10. กด **Close Session** → ยืนยัน → Badge เปลี่ยนเป็น "closed"
